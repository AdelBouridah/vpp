...
getPodNames() {
        pods=`kubectl get pods | grep cnf`
        rpcnf1=`kubectl get pods |  grep -oE "cnf1-sfc3\S*"`
	rpcnf2=`kubectl get pods |  grep -oE "cnf2-sfc3\S*"`
	rpcnf3=`kubectl get pods |  grep -oE "cnf3-sfc3\S*"`
	rpcnf4=`kubectl get pods |  grep -oE "cnf4-sfc3\S*"`
	rpcnf5=`kubectl get pods |  grep -oE "cnf5-sfc3\S*"`
}
...
setuprpcnf5() {
	...
	for line in $rpcnf5; do  
		...
		kubectl exec $line -- ip link set dev tap1 up
		echo "Configured cnf5 with IP 192.168.187.2 for copy"$i;  
	done
}
