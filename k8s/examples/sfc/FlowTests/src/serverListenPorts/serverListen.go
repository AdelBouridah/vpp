...
func main() {
	...
	fmt.Println(" **************************************************************************************************")
	//var int endPorts
	beginPorts, _ := strconv.Atoi(os.Args[1])
	endPorts, _ := strconv.Atoi(os.Args[2])
	...
	for i := 0; i <= len(outputsCNFs)-2; i++ {
		if (endPorts<=9){
			for j := beginPorts; j <= endPorts; j++ {
				...
				fmt.Println("CNF listen on  port  ", outputsCNFs[i] , " on port 500"+strconv.Itoa(j))
			}
		}else{
			if (beginPorts<9){
			...
			for j := beginPorts; j <= endPorts; j++ {
				//fmt.Println("cnf outputs	", outputsCNFs[i])
				cmd := exec.Command("kubectl", "exec", outputsCNFs[i] , "--","iperf", "-s", "-p", "50"+strconv.Itoa(j), "-u")
				//cmd := exec.Command("kubectl", "get", "pods", )
				err := cmd.Start()
				if err != nil {
					log.Fatalf("cmd.Run() failed with %s\n", err)
				...
				//fmt.Printf("combined out:\n%s\n", string(out))
				fmt.Println("CNF listen on  port  ", outputsCNFs[i] , " on port 50"+strconv.Itoa(j))
			}
