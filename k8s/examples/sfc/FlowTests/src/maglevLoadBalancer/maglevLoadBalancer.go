...
func main() {
	...
	*/
	...
	fmt.Println(" *************************************************************************")
	fmt.Println(" ******			Create the Maglev                          **********")
	fmt.Println(" *************************************************************************")
	paths:=make([]string,len(inputsCNFs)-1)// -1 to avoid blanc end  ligne

	for i := 0; i <= len(inputsCNFs)-2; i++ {
		paths[i] = fmt.Sprintf( inputsCNFs[i])
	}

	k := maglev.New(paths, maglev.SmallM)
  ...
  flowArray:= make([]flow, 0)
	...
	for pqFlows.Len() > 0 {
		...
		rand.Seed(0)
		s := inputsCNFs[k.Lookup(uint64(rand.Int63()))]
