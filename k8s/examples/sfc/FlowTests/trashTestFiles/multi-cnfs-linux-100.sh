...
getPodNames() {
        ...
        cnf1=`echo "${pods}" | grep -oE "linux-cnf100\S*"`
        cnf2=`echo "${pods}" | grep -oE "linux-cnf101\S*"`
        cnf3=`echo "${pods}" | grep -oE "linux-cnf102\S*"`
        cnf4=`echo "${pods}" | grep -oE "linux-cnf103\S*"`
        cnf5=`echo "${pods}" | grep -oE "linux-cnf104\S*"`
}
...
checkReadiness() {
        ...
        duration=0
        node2=`kubectl get pods -o wide | grep linux-cnf101 | grep -m 1 -oE "k8s-worker\S*"`

        vswitch=`kubectl get pods -n kube-system -o wide | grep ${node2} | grep -oE "contiv-vswitch\S*"`
        until [[ `kubectl exec ${vswitch} -n kube-system -- /usr/bin/vppctl show int address 2>error.log | grep -o "xconnect" | wc -l` == 6 ]];do
...
setupcnf3() {
        kubectl exec ${cnf3} -- brctl addbr br1
        kubectl exec ${cnf3} -- brctl addif br1 tap1
        kubectl exec ${cnf3} -- brctl addif br1 tap2
        kubectl exec ${cnf3} -- ip link set dev br1 up
        kubectl exec ${cnf3} -- ip link set dev tap1 up
        kubectl exec ${cnf3} -- ip link set dev tap2 up
        echo "Configured cnf3 with linux bridge."
}

setupcnf4() {
        kubectl exec ${cnf4} -- brctl addbr br1
        kubectl exec ${cnf4} -- brctl addif br1 tap1
        kubectl exec ${cnf4} -- brctl addif br1 tap2
        kubectl exec ${cnf4} -- ip link set dev br1 up
        kubectl exec ${cnf4} -- ip link set dev tap1 up
        kubectl exec ${cnf4} -- ip link set dev tap2 up
        echo "Configured cnf4 with linux bridge."
}

setupcnf5() {
        kubectl exec ${cnf5} -- ip address add 192.168.187.2/30 dev tap1
        kubectl exec ${cnf5} -- ip link set dev tap1 up
