...
getPodNames() {
        ...
        rpcnf1=`kubectl get pods |  grep -oE "ft2-rep-linux-cnf1\S*"`
	rpcnf2=`kubectl get pods |  grep -oE "ft2-rep-linux-cnf2\S*"`
	rpcnf3=`kubectl get pods |  grep -oE "ft2-rep-linux-cnf3\S*"`
	rpcnf4=`kubectl get pods |  grep -oE "ft2-rep-linux-cnf4\S*"`
	rpcnf5=`kubectl get pods |  grep -oE "ft2-rep-linux-cnf5\S*"`
	rpcnf6=`kubectl get pods |  grep -oE "ft2-rep-linux-cnf6\S*"`
	rpcnf7=`kubectl get pods |  grep -oE "ft2-rep-linux-cnf7\S*"`
}

checkReadiness() {
        echo "Checking pod readiness...(up to 30sec)"
        duration=0
        until [[ `kubectl get pods | grep ft2-rep-linux-cnf` != *"0/1"* ]];do
...
testFlowsOverPaths(){
	...
	while (($i<100)); 	do
		j=(RANDOM%7)
		kubectl exec ${cnfbegintab[$j]} -- ping -c 2 -I tap1 192.168.187.2
